<template>
  <div
    ...
    <xx-input
      ...
      <template slot="suffix">
        <i
          ...
          key="clear"
          class="xx-input__icon xx-icon-circle-close"
    ...
    <div v-if="multiple" class="xx-cascader__tags">
      <xx-tag
      ...
      <input
        ...
        type="text"
        class="xx-cascader__search-input"
    ...
    <transition name="xx-zoom-in-top" @after-leave="handleDropdownLeave">
      <div
        v-show="dropDownVisible"
        ref="popper"
        :class="['xx-popper', 'xx-cascader__dropdown', popperClass]">
        <xx-cascader-panel
          ref="panel"
          v-show="!filtering"
          ...
          @close="toggleDropDownVisible(false)"></xx-cascader-panel>
        <xx-scrollbar
          ref="suggestionPanel"
          v-if="filterable"
          v-show="filtering"
          tag="ul"
          class="xx-cascader__suggestion-panel"
          view-class="xx-cascader__suggestion-list"
          ...
          <template v-if="suggestions.length">
            <li
              ...
              :key="item.uid"
              :class="[
                'xx-cascader__suggestion-item',
                item.checked && 'is-checked'
              ]"
              :tabindex="-1"
              @click="handleSuggestionClick(index)">
              <span>{{ item.text }}</span>
              <i v-if="item.checked" class="xx-icon-check"></i>
            </li>
          </template>
          <slot v-else name="empty">
            <li class="xx-cascader__empty-text">无匹配数据</li>
          </slot>
        </xx-scrollbar>
      </div>
    </transition>
...
import Migrating from 'xxqx-ui/src/mixins/migrating';
...
import XxCascaderPanel from 'xxqx-ui/packages/cascader-panel';
import AriaUtils from 'xxqx-ui/src/utils/aria-utils';
import { isEqual, isEmpty, kebabCase } from 'xxqx-ui/src/utils/util';
import { isUndefined, isFunction } from 'xxqx-ui/src/utils/types';
import { isDef } from 'xxqx-ui/src/utils/shared';
...
export default {
  ...
  methods: {
    ...
    focusFirstNode() {
      this.$nextTick(() => {
        ...
        if (filtering && suggestionPanel) {
          firstNode = suggestionPanel.$el.querySelector('.xx-cascader__suggestion-item');
        } else {
          const firstMenu = popper.querySelector('.xx-cascader-menu');
          firstNode = firstMenu.querySelector('.xx-cascader-node[tabindex="-1"]');
        }
    ...
    updateStyle() {
      ...
      const inputInner = $el.querySelector('.xx-input__inner');

      if (!inputInner) return;

      const tags = $el.querySelector('.xx-cascader__tags');
      let suggestionPanelEl = null;

      if (suggestionPanel && (suggestionPanelEl = suggestionPanel.$el)) {
        const suggestionList = suggestionPanelEl.querySelector('.xx-cascader__suggestion-list');
