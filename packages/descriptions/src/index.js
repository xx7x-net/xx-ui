...
import { isFunction } from 'xxqx-ui/src/utils/types'
...
export default {
  ...
  methods: {
    ...
    getRows() {
      const children = ((this.$slots.default || []).filter(vnode => vnode.tag &&
            vnode.componentOptions && vnode.componentOptions.Ctor.options.name === 'XxDescriptionsItem'))
  ...
  render() {
    ...
    return (
      <div class="xx-descriptions">
        {
          (title || extra || $slots.title || $slots.extra)
            ? <div class="xx-descriptions__header">
              <div class="xx-descriptions__title">
                { $slots.title ? $slots.title : title}
              </div>
              <div class="xx-descriptions__extra">
        ...
        <div class="xx-descriptions__body">
          <table class={['xx-descriptions__table', { 'is-bordered': border }, descriptionsSize ? `xx-descriptions--${descriptionsSize}` : '']}>
            {rows.map(row => (
