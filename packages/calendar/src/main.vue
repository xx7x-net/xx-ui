<template>
  <div class="xx-calendar">
    <div class="xx-calendar__header">
      <div class="xx-calendar__title">
        {{ date.getFullYear() }}
        年
      ...
      <div
        class="xx-calendar__button-group"
  ...
  methods: {
    ...
    rangeValidator(date, isStart) {
      ...
      if (date.getDay() !== expected) {
        console.warn('[Calendar]', message, 'Invalid range will be ignored.');
        return false;
      }
      return true;
    }
  ...
  computed: {
    ...
    validatedRange() {
      ...
      if (range.length === 2) {
        ...
        if (start > end) {
          console.warn('[Calendar]end time should be greater than start time');
          return [];
        }
        ...
        if (!validateRangeInOneMonth(startDay, end)) {
          console.warn('[Calendar]start time and end time interval must not exceed two months');
          return [];
        }
        // 第一个月的时间范围
        data.push([
