...
import { range as rangeArr, getFirstDayOfMonth, getPrevMonthLastDays, getMonthDays, validateRangeInOneMonth } from 'xxqx-ui/src/utils/date-util';
...
export default {
  ...
  methods: {
    ...
    cellRenderProxy({ text, type }) {
      let render = this.xxCalendar.$scopedSlots.dateCell;
  ...
  computed: {
    ...
    formatedToday() {
      return this.xxCalendar.formatedToday;
    },
        ...
        <tbody>
          {
            this.rows.map((row, index) => <tr
              class={{
                'xx-calendar-table__row': true,
                'xx-calendar-table__row--hide-border': index === 0 && this.hideHeader
              }}
              key={index}>
              {
