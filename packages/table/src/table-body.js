import { arrayFindIndex } from 'xxqx-ui/src/utils/util'
import { getCell, getColumnByCell, getRowIdentity } from './util'
import { getStyle, hasClass, removeClass, addClass } from 'xxqx-ui/src/utils/dom'
import XxCheckbox from 'xxqx-ui/packages/checkbox'
import XxTooltip from 'xxqx-ui/packages/tooltip'
...
export default {
    ...
    return (
      <table
        ...
        <tbody>
          ...
          <xx-tooltip effect={this.table.tooltipEffect} placement="top" ref="tooltip" content={this.tooltipContent}></xx-tooltip>
        </tbody>
      </table>
    )
  },
  ...
  watch: {
    ...
    'store.states.hoverRow'(newVal, oldVal) {
      ...
      raf(() => {
        const rows = this.$el.querySelectorAll('.xx-table__row')
  ...
  methods: {
    ...
    getRowClass(row, rowIndex) {
      const classes = ['xx-table__row']
      if (this.table.highlightCurrentRow && row === this.store.states.currentRow) {
        classes.push('current-row')
      }

      if (this.stripe && rowIndex % 2 === 1) {
        classes.push('xx-table__row--striped')
      }
    ...
    handleCellMouseEnter(event, row) {
      ...
      if (!(hasClass(cellChild, 'xx-tooltip') && cellChild.childNodes.length)) {
        return
      }
    ...
    rowRender(row, $index, treeRowData) {
      ...
      let display = true
      if (treeRowData) {
        rowClasses.push('xx-table__row--level-' + treeRowData.level)
      ...
      if (this.hasExpandColumn && isRowExpanded(row)) {
        ...
        return [[
          ...
          <tr key={'expanded-row__' + tr.key}>
            <td colspan={ this.columnsCount } class="xx-table__cell xx-table__expanded-cell">
